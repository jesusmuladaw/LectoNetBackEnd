...
export default function Show({ auth, mustVerifyEmail, status, book, reading_status, ownership_status, usersWithBookToShare }) {
    ...
    const handleRequestBook = async (lenderId) => {
        try {
            await axios.post(`/books/${book.id}/request-loan`, {
                lender_id: lenderId,
            });
            ...
            <div className="container mx-auto px-4 pt-10">
                <div className="bg-white rounded-lg shadow p-6 flex flex-wrap md:flex-nowrap">
                    ...
                    <div className="w-full md:w-2/3 md:pl-6 mt-6 md:mt-0">
                        ...
                        <div className="mt-6">
                            <h2 className="font-bold text-lg mb-2">Usuarios que tienen este libro para compartir:</h2>
                            {usersWithBookToShare.length > 0 ? (
                                <ul>
                                    {usersWithBookToShare.map(user => (
                                        <li key={user.id} className="mb-2">
                                            <a href={`/profile/${user.id}`} className="text-blue-500 hover:underline">{user.name} {user.apellidos}</a>
                                            <button
                                                onClick={() => handleRequestBook(user.id)}
                                ...
                                <p>No hay usuarios compartiendo este libro.</p>
                            )}
                        </div>
                    </div>
                </div>
